    ...
    def run(self):
        ...
        while self.is_listening:
            ...
            try:
                ...
                try:
                    if room_id not in list(self.rooms.rooms.keys()):
                        raise RoomNotFound
                    self.lock.acquire()
                    try:
                        if action == "send":
...
class TcpServer(Thread):
    ...
    def route(self,
        ...
        if identifier is not None:
            if identifier not in list(self.rooms.players.keys()):
            ...
            if action == "join":
                try:
                    if payload not in list(self.rooms.rooms.keys()):
                        raise RoomNotFound()
                ...
                rooms = []
                for id_room, room in list(self.rooms.rooms.items()):
